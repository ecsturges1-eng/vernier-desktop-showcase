...
// Four stages rather than five, so each block takes the width the trimmed
// set frees up.
const N = 4;
const STAGE_W = 224, GAP = 56, STAGE_Y = 296, STAGE_H = 268;
const X0 = Math.round((W - (STAGE_W * N + GAP * (N - 1))) / 2);
...
const RAIL = { x: X0, y: 208, w: STAGE_W * N + GAP * (N - 1), h: 8 };

function MonitorTrim({ tw }) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <span style={{ flex: 1 }} />
        {compact ? null : (
          <span style={{ font: t.micro, letterSpacing: TRACK, color: C.label }}>FOUR STAGES · NO EXCEPTIONS</span>
        )}
      </div>
    </div>
  );
...
window.MonitorMobile2Piece = MonitorMobile2Piece;
