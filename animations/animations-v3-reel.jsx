...
function Stage({
  ...
  // Measures the playback bar's own rendered height instead of assuming a
  // fixed 44px — production embeds hide the bar via
  // `[data-omelette-chrome]{display:none}`, and a hidden bar has zero
  // height, so the canvas gets the full container with no dead margin.
  ...
  React.useEffect(() => {
    ...
    const measure = () => {
      const barH = barRef.current ? barRef.current.clientHeight : 44;
      const s = Math.min(
