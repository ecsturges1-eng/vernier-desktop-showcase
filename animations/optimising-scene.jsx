...
const { MetricCard, Badge, Delta, Button } = DS;
...
const ROW_H = 84, F_HEAD = 34;
const HIST_T = 288, HIST_H = 52;
...
const SIGNALS = [
  { src: "SCHEME FEES", text: "Scheme fee bulletin · +0.02% cross-border", impact: 0.82, conf: 0.96, exp: 0.48, row: 1, dTake: 0.40, dMargin: 0.06, dIdx: 2, find: "Recoverable at +0.40bps" },
  { src: "COMPETITOR", text: "Enterprise MDR repriced up 3.00bps", impact: 0.74, conf: 0.88, exp: 0.61, row: 0, dTake: 0.60, dMargin: 0.14, dIdx: 3, find: "0.60bps headroom, within tolerance" }
];
...
const TREND_REV = scaleTo(shape(24, 0.17, 0.010, 1.7), REV0);
...
function Link({ x1, y1, x2, y2, draw }) {
  ...
  return (
    <g opacity={clamp(draw * 6, 0, 1)}>
      <rect x={x1 - 4} y={y1 - 4} width="8" height="8" fill={C.accent} />
      <path d={d} fill="none" stroke={C.accent} strokeWidth="1.5" strokeLinejoin="miter"
            pathLength="1" strokeDasharray="1" strokeDashoffset={1 - draw} />
      <g opacity={land}>
        <rect x={x2 - 4} y={y2 - 4} width="8" height="8" fill={C.accent} />
        <rect x={x2} y={y2 - 1} width="10" height="2" fill={C.accent} />
      </g>
    </g>
  );
}
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 16, height: ROW_H, boxSizing: "border-box", padding: "0 18px", borderTop: `1px solid ${C.border}`, background: `rgba(238,241,250,${clamp(flag, 0, 1).toFixed(3)})` }}>
      <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: 5 }}>
        ...
        <span style={{ ...MONO_XS, color: C.label }}>{row.ref} · £{row.vol.toFixed(1)}BN</span>
      </div>
      ...
      <span style={{ position: "relative", width: 96, height: 22, flex: "none" }}>
        <span style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "flex-end", ...MONO_XS, color: C.label, opacity: 1 - clamp(flag * 1.6, 0, 1) }}>STABLE</span>
        <span style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "flex-end", opacity: clamp((flag - 0.35) / 0.4, 0, 1) }}>
          <Badge tone="accent" mono style={CHIP}>REVIEW</Badge>
        </span>
      </span>
    </div>
  );
  ...
  return (
    <div style={{ position: "absolute", inset: 0, background: C.bg, overflow: "hidden", fontFamily: "var(--font-sans)" }}>
      ...
      <div style={{ position: "absolute", left: PAD, top: 48, width: INNER, display: "flex", alignItems: "center", gap: 24 }}>
        <span style={{ ...MONO_XS, color: C.label }}>VERNIER · PAYMENTS</span>
        <span style={{ flex: 1 }} />
        <span style={{ ...MONO_SM, color: C.text }}>{clock}</span>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <Button variant="secondary" size="sm" style={{ borderRadius: R }}
                  iconLeft={<span style={{ ...MONO_XS, color: C.accent }}>AI</span>}>Ask Vernier</Button>
          <Button variant="secondary" size="sm" style={{ borderRadius: R }}
                  iconLeft={<span style={{ ...MONO_XS, color: C.accent }}>+</span>}>Product / fee type</Button>
          <Button variant="secondary" size="sm" style={{ borderRadius: R }}>My account</Button>
        </div>
      </div>

      {/* ── Aggregates ── */}
      <div style={{ position: "absolute", left: PAD, top: 120, width: INNER, display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 24 }}>
        <MetricCard label="Net revenue, annualised" value={`£${rev.toFixed(2)}m`}
                    footnote={<Delta value={rev - rev0} unit="m" size="sm" />} style={{ borderRadius: R, height: 148, boxSizing: "border-box" }} />

        <Tile label="NET REVENUE · £M" meta="24 MONTHS">
          <span style={{ font: "400 24px/1 var(--font-mono)", color: C.text, fontVariantNumeric: "tabular-nums" }}>{TREND_REV[23].toFixed(2)}</span>
          <Spark series={TREND_REV} w={252} h={44} draw={trendDraw} />
        </Tile>

        <Tile label="MARGIN / TXN · P" meta="24 MONTHS">
        ...
        <Tile label="ATTRITION RISK" meta="OUT OF 100">
          <div style={{ display: "flex", alignItems: "baseline", gap: 12 }}>
            ...
            <Badge tone={ib.tone} mono style={CHIP}>{ib.word}</Badge>
          </div>
      ...
      <div style={{ position: "absolute", left: PAD, top: PIPE_T, width: INNER, height: COLH }}>
        <div style={{ position: "absolute", left: X_S, top: 0, width: COL_S }}>
          <Panel label="SIGNALS" meta={`${String(SIGNALS.filter((x, j) => ph[j].card > 0.05).length).padStart(2, "0")} / 02`}>
            {SIGNALS.map((sig, j) => (
          ...
          <Panel label="ANALYSIS">
            <div style={{ position: "absolute", inset: 0, padding: "18px 18px 16px", display: "flex", flexDirection: "column", gap: 20, boxSizing: "border-box" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                <span style={{ ...MONO_XS, color: C.label }}>IMPACT × CONFIDENCE</span>
                <div style={{ display: "flex", alignItems: "baseline", gap: 12 }}>
                  ...
                  <span style={{ opacity: p.badge }}><Badge tone="accent" mono style={CHIP}>REVIEW</Badge></span>
                </div>
                <span style={{ ...MONO_XS, color: C.label }}>THRESHOLD 0.55</span>
                <span style={{ font: "400 14px/1.45 var(--font-sans)", color: C.positive, height: 40, overflow: "hidden", opacity: p.badge }}>{s.find}</span>
              </div>
            </div>
          </Panel>
        </div>
        ...
        <div style={{ position: "absolute", left: X_F, top: 0, width: COL_F }}>
          <Panel label="FEE BOOK" meta={`${String(flagged).padStart(2, "0")} FOR REVIEW`}>
            <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 16, height: F_HEAD, flex: "none", padding: "0 18px" }}>
                ...
                <span style={{ ...MONO_XS, width: 84, flex: "none", textAlign: "right", color: C.label }}>NET TAKE BPS</span>
                <span style={{ ...MONO_XS, width: 84, flex: "none", textAlign: "right", color: C.label }}>MARGIN P</span>
                <span style={{ ...MONO_XS, width: 96, flex: "none", textAlign: "right", color: C.label }}>STATUS</span>
              </div>
      ...
      <div style={{ position: "absolute", left: PAD, top: 852, width: INNER, display: "flex", alignItems: "baseline" }}>
        <span style={{ ...MONO_SM, color: C.text }}>
          <span style={{ font: "600 15px/1.2 var(--font-mono)", letterSpacing: "0.06em", color: C.positive, fontVariantNumeric: "tabular-nums" }}>£{(rev - REV0).toFixed(2)}M</span>
          <span style={{ marginLeft: 8 }}>ANNUAL NET REVENUE IDENTIFIED</span>
        </span>
        <span style={{ ...MONO_XS, color: C.label, marginLeft: 14 }}>{String(flagged).padStart(2, "0")} STRUCTURES · CONFIDENCE 0.92</span>
        <span style={{ flex: 1 }} />
        <span style={{ ...MONO_XS, color: C.label }}>1,284 VARIABLES · 6 SOURCES</span>
      </div>
    </div>
  );
}
